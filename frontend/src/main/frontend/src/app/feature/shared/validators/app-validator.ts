...
export class AppValidators {
 ...
 static minLength(minimum) {
    return function(input) : ValidationErrors {
      return input.value.length >= minimum ? null : { minLength: minimum };
    };
  }
  static maxLength(maximum) {
    const message = {
      'maxLength': {
    ...
    return function(input) : ValidationErrors {
      return input.value.length <= maximum ? null : message;
    };
  }
